...
func main() {
	...
	// Call Create
	resp, err := http.Post(*address+"/v1/user", "application/json", strings.NewReader(fmt.Sprintf(`
		{
			"api":"v1",
			"user": {
				"nome":"nome (%s)",
	...
	// Call Read
	resp, err = http.Get(fmt.Sprintf("%s%s/%s", *address, "/v1/user", created.ID))
	if err != nil {
	...
	// Call Update
	req, err := http.NewRequest("PUT", fmt.Sprintf("%s%s/%s", *address, "/v1/user", created.ID),
		strings.NewReader(fmt.Sprintf(`
		{
			"api":"v1",
			"user": {
				"nome":"nome (%s) + updated",
				"sobrenome":"sobrenome (%s) + updated",
				"reminder":"%s"
			}
		}
	`, pfx, pfx, pfx)))
	...
	// Call ReadAll
	resp, err = http.Get(*address + "/v1/user/all")
	if err != nil {
	...
	// Call Delete
	req, err = http.NewRequest("DELETE", fmt.Sprintf("%s%s/%s", *address, "/v1/user", created.ID), nil)
	resp, err = http.DefaultClient.Do(req)
	if err != nil {
